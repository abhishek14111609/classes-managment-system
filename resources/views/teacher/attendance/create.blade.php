                            ...
                            <div class="row g-3 align-items-end">
                                <div class="col-md-5">
                                    ...
                                    <div class="input-group overflow-hidden rounded-4 border">
                                        ...
                                        <select class="form-select border-0 py-3 shadow-none bg-white fw-bold" name="batch_id"
                                            onchange="this.form.submit()">
                                            <option value="">Awaiting Batch Selection...</option>
                                            @foreach ($batches as $batch)
                                                <option value="{{ $batch->id }}"
                                                    {{ request('batch_id') == $batch->id ? 'selected' : '' }}>
                                                    {{ $batch->name }} ({{ $batch->start_time ? $batch->start_time->format('h:i A') : 'N/A' }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group overflow-hidden rounded-4 border">
                                        ...
                                        <input type="date" class="form-control border-0 py-3 shadow-none bg-white fw-bold"
                    ...
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div class="card-header bg-white border-0 pt-4 px-4 pb-0 d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="text-muted small">Target Batch: {{ $batches->firstWhere('id', request('batch_id'))->name ?? 'Selected' }}</p>
                            </div>
                            <button type="button" class="btn btn-success btn-sm rounded-pill px-4 py-2 shadow-sm fw-bold border-0"
                            ...
                            <form action="{{ route('teacher.attendance.store') }}" method="POST">
                                    ...
                                    <table class="table table-hover align-middle mb-0">
                                        ...
                                        <tbody>
                                            @foreach ($students as $student)
                                                @php
                                                    $existingAttendance = isset($attendanceRecords) ? $attendanceRecords->where('student_id', $student->id)->first() : null;
                                                @endphp
                                                <tr class="{{ $existingAttendance && $existingAttendance->verification_status === 'pending' ? 'bg-primary bg-opacity-5' : '' }}">
                                                    <td class="ps-4 py-4 border-0">
                                                        <div class="d-flex align-items-center">
                                                            <div class="avatar-container position-relative me-3">
                                                                @if ($student->user->avatar)
                                                                    <img src="{{ asset('storage/' . $student->user->avatar) }}" class="rounded-circle shadow-sm border" style="width: 48px; height: 48px; object-fit: cover;">
                                                                @else
                                                                    <div class="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 48px; height: 48px; border: 2px solid white;">
                                                                        {{ strtoupper(substr($student->user->name, 0, 1)) }}
                                                                    </div>
                                                                @endif
                                                                @if ($existingAttendance && $existingAttendance->photo_path)
                                                                    <span class="position-absolute bottom-0 end-0 bg-info rounded-circle border-2 border-white" style="width: 14px; height: 14px;" title="Media Uploaded"></span>
                                                                @endif
                                                            </div>
                                                            <div>
                                                                <div class="d-flex align-items-center mb-1">
                                                                    <h6 class="fw-bold text-dark mb-0 me-2">{{ $student->user->name }}</h6>
                                                                    @if ($existingAttendance && $existingAttendance->photo_path)
                                                                        <button type="button" class="badge bg-primary bg-opacity-10 text-primary border-0 rounded-pill px-2 py-1 tiny fw-bold cursor-pointer"
                                                                ...
                                                                @if ($existingAttendance && $existingAttendance->verification_status === 'pending')
                                                                    <div class="d-flex align-items-center gap-2 mt-1">
                                                                        <span class="badge bg-warning text-dark tiny fw-bold animate-pulse">
                                                                            <i class="bi bi-exclamation-triangle-fill me-1"></i> VERIFICATION REQ
                                                                        </span>
                                                                        <small class="text-primary tiny fw-bold">
                                                                            <i class="bi bi-clock-fill me-1"></i> {{ $existingAttendance->photo_submitted_at ? $existingAttendance->photo_submitted_at->format('h:i A') : 'N/A' }}
                                                                        </small>
                                                                    </div>
                                                                @else
                                                                    <small class="text-muted tiny fw-bold mt-1 d-block">{{ $student->user->email }}</small>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="border-0">
                                                        <code class="text-primary fw-bold">#{{ $student->roll_number }}</code>
                                                    </td>
                                                    <td class="border-0 text-center">
                                                        <input type="hidden" name="attendances[{{ $student->id }}][student_id]" value="{{ $student->id }}">
                                                        <div class="btn-group p-1 bg-light rounded-pill border overflow-hidden shadow-sm" style="height: 48px;">
                                                            @php
                                                                $isPending = $existingAttendance && $existingAttendance->verification_status === 'pending';
                                                            @endphp
                                                            
                                                            <input type="radio" class="btn-check" name="attendances[{{ $student->id }}][status]" value="present" id="present{{ $student->id }}"
                                                            ...
                                                            <label class="btn btn-modern-status btn-p rounded-pill d-flex align-items-center justify-content-center px-4" for="present{{ $student->id }}">
                                                                <span class="fw-bold">{{ $isPending ? 'APPROVE' : 'PRESENT' }}</span>
                                                            </label>

                                                            <input type="radio" class="btn-check" name="attendances[{{ $student->id }}][status]" value="absent" id="absent{{ $student->id }}"
                                                                {{ $existingAttendance && $existingAttendance->status === 'absent' ? 'checked' : '' }}>
                                                            <label class="btn btn-modern-status btn-a rounded-pill d-flex align-items-center justify-content-center px-4" for="absent{{ $student->id }}">
                                                                <span class="fw-bold">{{ $isPending ? 'REJECT' : 'ABSENT' }}</span>
                                                            </label>

                                                            <input type="radio" class="btn-check" name="attendances[{{ $student->id }}][status]" value="late" id="late{{ $student->id }}"
                                                                {{ $existingAttendance && $existingAttendance->status === 'late' ? 'checked' : '' }}>
                                                            <label class="btn btn-modern-status btn-l rounded-pill d-flex align-items-center justify-content-center px-4" for="late{{ $student->id }}">
                                                    ...
                                                    <td class="pe-4 border-0">
                                                        <div class="input-group input-group-sm">
                                                            <span class="input-group-text bg-light border-0"><i class="bi bi-pencil-square opacity-50"></i></span>
                                                            <input type="text" class="form-control border-0 bg-light py-2 shadow-none px-3" name="attendances[{{ $student->id }}][remarks]" value="{{ $existingAttendance->remarks ?? '' }}" placeholder="Add performance metrics...">
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                                ...
                                <div class="mt-4 pt-4 border-top d-flex justify-content-between align-items-center">
                                    <div class="text-muted small">
                                        <i class="bi bi-info-circle me-1"></i> Marks for <strong>Presence</strong> will be auto-accepted for pending biometric uploads.
                                    </div>
                                    <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5 py-3 shadow-lg fw-bold bg-gradient-brand border-0">
            ...
            <div class="modal-content border-0 shadow-lg rounded-5 overflow-hidden bg-dark">
                ...
                <div class="modal-body p-4 text-center">
                    <div class="rounded-4 overflow-hidden shadow-lg border border-white border-opacity-10 position-relative">
                        <img src="" id="perspectiveImg" class="img-fluid" style="width: 100%; max-height: 65vh; object-fit: contain;">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3 bg-dark bg-opacity-50 backdrop-blur d-flex justify-content-between align-items-center text-white">
                            <small class="tiny fw-bold"><i class="bi bi-cpu me-1"></i> Physical Verification Record</small>
                            <small class="tiny fw-bold" id="perspectiveTime"><i class="bi bi-clock me-1"></i> SYMPTOM TIME: --:--</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-0 p-4">
                    <button type="button" class="btn btn-outline-white border-white border-opacity-25 rounded-pill px-5 fw-bold text-white" data-bs-dismiss="modal">Close Perspective</button>
                </div>
            </div>
        </div>
    </div>
...
<style>
    ...
    .btn-check:checked + .btn-p { background-color: #10b981 !important; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.4); }
    .btn-check:checked + .btn-a { background-color: #ef4444 !important; box-shadow: 0 4px 12px rgba(239, 68, 68, 0.4); }
    .btn-check:checked + .btn-l { background-color: #f59e0b !important; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.4); }
