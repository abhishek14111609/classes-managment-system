                ...
                <form action="{{ route('school.subjects.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="name"
                                class="form-label fw-semibold">{{ auth()->user()->school->institute_type === 'sport' ? 'Activity Name' : 'Subject Name' }}
                                <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name') }}"
                                placeholder="{{ auth()->user()->school->institute_type === 'sport' ? 'e.g. Endurance Training, Batting Practice' : 'e.g. Mathematics, Science' }}"
                                required>
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="class_id"
                                class="form-label fw-semibold">{{ auth()->user()->school->institute_type === 'sport' ? 'Team / Level' : 'Class / Level' }}
                                <span class="text-danger">*</span></label>
                            <select class="form-select @error('class_id') is-invalid @enderror" id="class_id"
                                name="class_id" required>
                                <option value="">—
                                    {{ auth()->user()->school->institute_type === 'sport' ? 'Select Team / Level' : 'Select Class / Level' }}
                                    —</option>
